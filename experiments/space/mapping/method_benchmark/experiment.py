...
def benchmark(cfg):
    ...
    def _tangram(
        ...
        tg.pp_adatas(adata_sc, adata_sp_train, genes=true_df.columns.tolist(), gene_to_lowercase=True)
    ...
    def _corr_results(true_df: pd.DataFrame, pred_df: pd.DataFrame) -> pd.DataFrame:
        pred_df = pred_df[true_df.index, :].copy()
    ...
    path_results = cfg.paths.path_results
    ...
    if method == "MOSCOT":
        params = {"epsilon": cfg.method.epsilon, "alpha": cfg.method.alpha}
        return _moscot(
            adata_sc=adata_sp_a,
